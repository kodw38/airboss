...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            // 弹出层指定宽度
            open: function (title, url, width, height, callback,bts) {
            	//如果是移动端，就使用自适应大小弹窗
            	if ($.common.isMobile()) {
            	    width = 'auto';
            	    height = 'auto';
        ...
        operate: {
            ...
            take: function(id) {
                table.set();
                $.modal.confirm("确定认领该条" + table.options.modalName + "信息吗？", function() {
                    var url = $.common.isEmpty(id) ? table.options.removeUrl : table.options.removeUrl.replace("{id}", id);
                    if(table.options.type == table_type.bootstrapTreeTable) {
                        $.operate.get(url);
                    } else {
            ...
            view: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open("查看" + table.options.modalName, $.operate.viewUrl(id),null,null,null,['关闭']);
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('._iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
