...
public class SysDeptController extends BaseController
{
    ...
    @PostMapping("/list")
    @ResponseBody
    public List<SecOrganizeBean> list(SecOrganizeBean dept)
    {
        List<SecOrganizeBean> deptList = deptService.selectDeptList(dept);
        return deptList;
    }
    ...
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult addSave(@Validated SecOrganizeBean dept)
    {
        if (deptService.checkDeptNameUnique(dept))
        {
            return error("新增部门'" + dept.getOrganizeName() + "'失败，部门名称已存在");
        }
    ...
    {
        SecOrganizeBean dept = deptService.selectDeptById(deptId);
    ...
    @PostMapping("/edit")
    @ResponseBody
    public AjaxResult editSave(@Validated SecOrganizeBean dept)
    {
        /*if (UserConstants.DEPT_NAME_NOT_UNIQUE.equals(deptService.checkDeptNameUnique(dept)))
        {
            return error("修改部门'" + dept.getOrganizeName() + "'失败，部门名称已存在");
        }
    ...
    {
        List<Ztree> ztrees = deptService.selectDeptTree(new SecOrganizeBean());
        return ztrees;
    }
